...
const NATIVE_START_COMMAND = "cmd.exe /c StartWindrosePlusServer.bat";
...
export function useWindrosePlus(instanceId: string, daemonId: string) {
  ...
  const enabled = computed(() => {
    ...
    return (config?.startCommand ?? "").toLowerCase().includes("startwindroseplusserver.bat");
  });
